...
import { GridBg, Style1Slide } from './primitives.jsx';
...
export function Style1_02Data() {
  const chartRows = colors.map(([, , color, pct, name, en]) => ({
    label: name,
    value: Number.parseFloat(pct) || 0,
    display: pct,
    tone: color,
    meta: en,
  }));
  ...
  return (
    <Style1Slide layout="ST1-02" className="st1-data">
      ...
      <ChartSwitch title="常用色彩占比" rows={chartRows} className="st1-chart-switch" />
